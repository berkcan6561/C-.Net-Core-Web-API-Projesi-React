  ...
  const handleDelete = (id: number) => {
   ...
   if (isLoading) return (
    <div className="flex flex-col items-center justify-center h-64 gap-4">
      ...
      <p className="text-slate-600 font-semibold animate-pulse">{t('pages.customers.loadingCustomers')}</p>
    </div>
  );

  if (isError) return (
    <div className="p-6 bg-red-50 border border-red-100 rounded-2xl text-red-500 text-center font-medium shadow-sm">
      {t('pages.customers.loadError')}
    </div>
  );
  ...
  return (
    <div className="animate-fade-in space-y-8 relative">
      <div className="flex justify-between items-end">
        <div>
          <div className="flex items-center gap-2 mb-2 text-slate-500 text-sm font-bold tracking-wider uppercase">
            <Users size={16} className="text-blue-600" /> {t('pages.customers.pageTitle')}
          </div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
            {t('pages.customers.pageHeader')}
          </h1>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-slate-50 text-slate-500 text-xs uppercase font-bold border-b border-slate-200">
              <tr>
                <th className="px-6 py-4">{t('pages.customers.tableCustomer')}</th>
                <th className="px-6 py-4">{t('pages.customers.tableContact')}</th>
                <th className="px-6 py-4 text-right">{t('pages.customers.tableActions')}</th>
              </tr>
            </thead>
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={editingCustomer ? t('pages.customers.editCustomer') : t('pages.customers.addCustomer')}
      >
        <CustomerForm
